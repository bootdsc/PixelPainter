#!/usr/bin/env python3
"""
Pixel Painter — dark-mode pixel art tool for game assets
Colors: dark gray, light gray, green, dark green only (color-blind friendly).

Project format: .ppix (JSON) — palette + pixels, pixel-perfect.
Export: PNG (lossless) for general use / intermediate pipeline.
"""
...
class PixelPainterApp:
    ...
    def cmd_about(self) -> None:
        messagebox.showinfo(
            ...
            (
                f"{APP_NAME}\n\n"
                "Project:  .ppix  (palette + indices, perfect for editing)\n"
                "Export PNG: lossless, use scale 1 for 1:1 pixels\n"
                "Export C: RGB565 arrays for ESP32 / M5GFX\n\n"
                "Index 0 = transparent on PNG / 0x0000 in C\n"
                "Canvas BG is editor-only (see empty vs painted cells).\n\n"
                "Space+drag: erase while Paint (trackpad-friendly).\n"
                "Box/Free select → Move selection → drag → Place.\n"
                "Off-grid placement is clipped on Place.\n"
                "Pick→slot: no popup (View menu for background).\n"
                "Color wheel: C  ·  Palette presets in menu.\n\n"
                "Keys: 1-4 brush  B/E/F/I tools  R box  L free  M move\n"
                "  G grid  P pick  C wheel  Esc clear select  Space invert\n"
                "  Alt+drag or middle-drag = pan  ·  Ctrl+/- zoom"
            ),
            parent=self.root,
        )
